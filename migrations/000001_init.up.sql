...
CREATE TABLE public.orders (
   ...
   starting_point_id uuid NOT NULL,
   finishing_point_id uuid NULL,

   -- Время начала аренды фиксируется БД
   rent_started_at timestamptz NOT NULL DEFAULT now(),

   -- Заполняется при завершении аренды
   rent_finished_at timestamptz NULL,

   -- Ограничения целостности
   CONSTRAINT order_product_fkey
      FOREIGN KEY (product_id)
      REFERENCES public.products(id)
      ON DELETE RESTRICT
      ON UPDATE CASCADE,

   CONSTRAINT order_starting_point_fkey
      FOREIGN KEY (starting_point_id)
      REFERENCES public.rentpoints(id)
      ON DELETE NO ACTION
      ON UPDATE CASCADE,

   CONSTRAINT order_finishing_point_fkey
      FOREIGN KEY (finishing_point_id)
      REFERENCES public.rentpoints(id)
      ON DELETE NO ACTION
      ON UPDATE CASCADE,

   -- Завершение не может быть раньше начала
   CONSTRAINT order_rent_time_check
      CHECK (
         rent_finished_at IS NULL
         OR rent_finished_at >= rent_started_at
      )
);
